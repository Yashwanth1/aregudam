...
<div class="sbox ">
	...
	<ul class="nav nav-tabs" >
	  <li class="active"><a href="#tab-sign-in" data-toggle="tab">  {{ Lang::get('core.signin') }} </a></li>
	   ...
	   @if(CNF_REGIST =='true') 			
	   <li><a href="{{ URL::to('/register')}}" >  {{ Lang::get('core.signup') }} </a></li>
	   @endif	
	 
	</ul>	
	<div class="tab-content" >
		<div class="tab-pane active m-t" id="tab-sign-in">
		<form method="post" action="{{ url('/signin')}}" class="form-vertical">
			<input type="hidden" name="_token" value="{{ csrf_token() }}">
		
			<div class="form-group has-feedback animated fadeInLeft delayp1">
				<label>{{ Lang::get('core.email') }}	</label>
				<input type="text" name="email_id" placeholder="Email Address" class="form-control" required="email" />
		...
		<div class="form-group has-feedback text-center">
			...
			<div style="padding:15px 0;">
				...
				<a href="{{ URL::to('admin/user/socialize/facebook')}}" class="btn btn-primary"><i class="icon-facebook"></i> Facebook </a>
				@endif
				@if($socialize['google']['client_id'] !='') 
				<a href="{{ URL::to('admin/user/socialize/google')}}" class="btn btn-danger"><i class="icon-google"></i> Google </a>
				@endif
				@if($socialize['twitter']['client_id'] !='') 
				<a href="{{ URL::to('admin/user/socialize/twitter')}}" class="btn btn-info"><i class="icon-twitter"></i> Twitter </a>
				@endif
			</div>
		</div>			
			  ...
			  <a href="{{ URL::to('admin/')}}"> {{ Lang::get('core.backtosite') }} </a>  
		   		</p>
		   	</div>	
		   </form>			
		</div>
	...
	<div class="tab-pane  m-t" id="tab-forgot">	
		...
		<form method="post" action="{{ url('/request')}}" class="form-vertical box" id="fr">
			<input type="hidden" name="_token" value="{{ csrf_token() }}">
