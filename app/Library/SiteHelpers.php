...
public static function alphaID($in, $to_num = false, $pad_up = false, $passKey = null)
{
	...
	public static  function transForm( $field, $forms = array(),$bulk=false , $value ='')
	{
		...
		switch($type)
		{
			...
			case 'select';
				...
				{
					if($option['lookup_table'] == "category" || $option['lookup_table'] == "sub_category")
					{
						$data = DB::table($option['lookup_table'])->where('status','active')->get();
					}
					else
					{
	...
	public static function createPreCombo( $field , $table , $key ,  $val ,$app ,$class ,$lookey = null)
	{
		...
		$pre_jCombo = "
		\$(\"#{$field}\").jCombo(\"{{ URL::to('admin/{$class}/comboselect?filter={$table}:{$key}:{$val}') }}$parent_field\",
	...
	public static function themeOption()
	{
		...
		$path = base_path().'/resources/admin/views/layouts/';
		$lang = scandir($path);
		$t = array();
		foreach($lang as $value) {
			if($value === '.' || $value === '..') {continue;} 
	...
	public static function BBCode2Html($text) {
		...
		$emotion =  URL::to('admin/sximo/js/plugins/markitup/images/emoticons/');
	...
	static function renderHtml( $html )
	{
		...
		$html = preg_replace( '/(\.+\/)+uploads/Usi' , URL::to('admin/uploads') ,  $html );
	//	$content = str_replace($pattern , URL::to('admin/').'/', $content );
	...
	public static function mainCategory($param)
	{
		if($param == 'category')
		{
			$getData = \DB::table( 'category')->select('pk_category_id','category_name')->get();
			return $getData;
		}
		else if($param == 'all')
		{
			$getData = \DB::table( 'sub_category')
						->select('pk_sub_category_id','sub_category_name','fk_category_id')
