...
abstract class Controller extends BaseController {
	...
	function postMultisearch( Request $request)
	{
		...
		endforeach;
		return Redirect::to('admin/'.$this->module.'?search='.substr($items,0,strlen($items)-1).'&md='.Input::get('md'));
	}
	...
	function postFilter( Request $request)
	{
		...
		return Redirect::to('admin/'.$this->data['pageModule'] . $filter);
	...
	function getDownload( Request $request)
	{
		...
		if($this->access['is_excel'] ==0) 
			return Redirect::to('admin/')->with('messagetext',\Lang::get('core.note_restric'))->with('msgstatus','error');   
